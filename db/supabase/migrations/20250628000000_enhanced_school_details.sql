...
ADD COLUMN IF NOT EXISTS school_type text default 'public',
...
CREATE INDEX IF NOT EXISTS idx_schools_email ON public.schools(email_address);
CREATE INDEX IF NOT EXISTS idx_schools_principal_email ON public.schools(principal_email);
...
-- RLS for school_analytics view - Note: RLS on views requires the underlying tables to have RLS
-- The policies will be inherited from the underlying tables

-- Update existing schools with default values
UPDATE public.schools 
SET 
    address = COALESCE(address, '{"street": "", "city": "", "state": "", "country": "", "postal_code": ""}'),
    theme_colors = COALESCE(theme_colors, '{"primary": "#2563eb", "secondary": "#64748b", "accent": "#0ea5e9"}'),
    settings = COALESCE(settings, '{"timezone": "UTC", "academic_year_start": "April", "working_days": ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]}');
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'schools_by_type', (
            ...
            FROM (
                SELECT COALESCE(school_type, 'unspecified') as school_type, COUNT(*) as count
                FROM public.schools
