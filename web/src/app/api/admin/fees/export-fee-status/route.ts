  ...
  try {
    ...
    const exportData = viewType === 'detailed'
      ? feeStatusData.map((record: any) => ({
          ...
          'Adjustment': parseFloat(record.adjustment_amount || record.discount_amount || 0).toFixed(2),
      ...
      : feeStatusData.map((record: any) => ({
          ...
          'Total Adjustment': parseFloat(record.total_adjustment_amount || record.total_discount_amount || 0).toFixed(2),
