...
/**
 * Calculate bulk attendance for multiple students (for report generation)
 */
export async function calculateBulkLast2MonthsAttendance(
  studentIds: string[],
  schoolId: string
): Promise<Record<string, AttendanceData>> {
  const results: Record<string, AttendanceData> = {};
  ...
  for (let i = 0; i < studentIds.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (studentId) => {
      const attendance = await calculateLast2MonthsAttendance(studentId, schoolId);
      return { studentId, attendance };
    });
    
    const batchResults = await Promise.all(batchPromises);
    batchResults.forEach(({ studentId, attendance }) => {
      results[studentId] = attendance;
    });
  }
...
/**
 * Calculate bulk monthly attendance for multiple students (for state board reports)
 */
export async function calculateBulkLast2MonthsMonthlyAttendance(
  studentIds: string[],
  schoolId: string
): Promise<Record<string, MonthlyAttendanceData>> {
  const results: Record<string, MonthlyAttendanceData> = {};
  ...
  for (let i = 0; i < studentIds.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (studentId) => {
      const attendance = await calculateLast2MonthsMonthlyAttendance(studentId, schoolId);
      return { studentId, attendance };
    });
