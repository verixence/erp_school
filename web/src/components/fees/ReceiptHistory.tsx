...
export default function ReceiptHistory({ schoolId }: { schoolId: string }) {
  ...
  const { data: receiptsData, isLoading } = useQuery({
    queryKey: ['receipts', schoolId, startDate, endDate],
    queryFn: async () => {
      let url = `/api/admin/fees/receipts?school_id=${schoolId}&limit=100`;
  ...
  const filteredReceipts = receipts.filter((receipt: Receipt) => {
    ...
    return (
      ...
      receipt.parent_name?.toLowerCase().includes(search)
    );
  });
        ...
        <div class="details">
          <table>
            ...
            <tr>
              ...
              <td>${receipt.parent_name}</td>
              ${receipt.parent_phone ? `
              <td><strong>Contact:</strong></td>
              <td>${receipt.parent_phone}</td>
              ` : '<td colspan="2"></td>'}
            </tr>
            ` : ''}
          </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <Input
                id="search"
                placeholder="Receipt No, Student, Parent..."
                value={searchTerm}
                  ...
                  <TableRow key={receipt.id}>
                    ...
                    <TableCell>
                      <div>
                        ...
                        <div className="text-sm text-gray-500">{receipt.student_admission_no}</div>
                      </div>
                    </TableCell>
                    <TableCell>{receipt.parent_name || '-'}</TableCell>
